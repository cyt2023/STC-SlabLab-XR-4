Shader "VolumeRendering/DirectVolumeRenderingShader"
{
    Properties
    {
        ...
        _JitterFactor("Ray jitter factor", Range(0.0, 5.0)) = 5.0
...
#define CROSS_SECTION_TYPE_SPHERE_EXCL 5
            ...
            frag_out frag_VolumeSTCube(frag_in i)
            {
                ...
                ray.startPos += (_JitterFactor * ray.direction * raymarchInfo.stepSize) * tex2D(_NoiseTex, float2(i.uv.x, i.uv.y)).r;
...
// #endif
            ...
            frag_out frag_surf(frag_in i)
            {
                ...
                // Create a small random offset in order to remove artifacts
                ray.startPos = ray.startPos + (_JitterFactor * ray.direction * raymarchInfo.stepSize) * tex2D(_NoiseTex, float2(i.uv.x, i.uv.y)).r;
