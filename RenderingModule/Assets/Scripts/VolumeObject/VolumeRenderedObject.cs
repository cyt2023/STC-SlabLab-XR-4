...
namespace UnityVolumeRendering
{
    ...
    public class VolumeRenderedObject : MonoBehaviour
    {
        ...
        private async Task UpdateMaterialPropertiesAsync(IProgressHandler progressHandler = null)
        {
            ...
            try
            {
                // A gradient volume is roughly four times the GPU size of the scalar
                // texture. Direct volume rendering only needs it for lighting (or a
                // 2D transfer function); creating it unconditionally quickly exhausts
                // Quest memory when time frames are changed.
                // Quest's normal STC view uses unlit direct-volume rendering. A
                // freshly created object can briefly inherit LIGHTING_ON from the
                // container prefab before the controller applies its preset. Do
                // not let that transient state allocate a full RGBA gradient
                // volume on Android; it can consume hundreds of MB per time frame.
