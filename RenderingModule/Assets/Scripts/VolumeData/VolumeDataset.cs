...
namespace UnityVolumeRendering
{
    ...
    public class VolumeDataset : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        // Optional source-domain window used when creating the display texture.
        // It changes rendering only; the source data array remains untouched.
        [NonSerialized]
        public float normalizationMinimum = float.NaN;
        [NonSerialized]
        public float normalizationMaximum = float.NaN;
        ...
        /// <summary>
        /// Releases runtime-only GPU textures owned by this dataset. Destroying the
        /// ScriptableObject alone does not reliably reclaim these generated textures
        /// before the next large time frame is uploaded on standalone XR devices.
        /// </summary>
        public void ReleaseRuntimeTextures()
        {
            if (dataTexture != null)
            {
        ...
        private async Task<Texture3D> CreateTextureInternalAsync(IProgressHandler progressHandler)                                        
        {
            ...
            await Task.Run(() =>
            {
                minValue = float.IsNaN(normalizationMinimum)
                    ? GetMinDataValue()
                    : normalizationMinimum;
                maxValue = float.IsNaN(normalizationMaximum)
                    ? 223.0f
                ...
                if (isHalfFloat)
                {
                    ...
                    await Task.Run(() => {
                        for (int i = 0; i < dimension;)
                        {
                            ...
                            for (int j = 0; j < sliceDimension; j++, i++)
                            {
                                float sourceValue = (float)data[i];
                                float normalized = sourceValue <= 0.0f && minValue > 0.0f
                                    ? 0.0f
                                    : Mathf.Clamp01((sourceValue - minValue) / maxRange);
                                pixelBytes[i] = Mathf.FloatToHalf(normalized);
                            }
                        }
                    });
                    progressHandler.EndStage();
                ...
                else
                {
                    ...
                    await Task.Run(() => {
                        for (int i = 0; i < dimension;)
                        {
                            ...
                            {
                                float sourceValue = (float)data[i];
                                pixelBytes[i] = sourceValue <= 0.0f && minValue > 0.0f
                                    ? 0.0f
                                    : Mathf.Clamp01((sourceValue - minValue) / maxRange);
                            }
                        }
                ...
                for (int x = 0; x < dimX; x++)
                {
                    ...
                    for (int y = 0; y < dimY; y++)
                    {
                        for (int z = 0; z < dimZ; z++)
                        {
                            ...
                            float sourceValue = (float)data[iData];
                            float normalized = sourceValue <= 0.0f && minValue > 0.0f
                                ? 0.0f
                                : Mathf.Clamp01((sourceValue - minValue) / maxRange);
                            textureTmp.SetPixel(x, y, z, new Color(grad.x, grad.y, grad.z, normalized));
                        }
                    }
                }
                    ...
                    for (int y = 0; y < dimY; y++)
                    {
                        for (int x = 0; x < dimX; x++)
                        {
                            ...
                            float normalized = sourceValue <= 0.0f && minValue > 0.0f
                                ? 0.0f
                                : Mathf.Clamp01((sourceValue - minValue) / maxRange);
                            cols[iData] = new Color(grad.x, grad.y, grad.z, normalized);
                        }
                    }
                    //for (int y = 0; y < dimY / 2; y++)
                    //{
